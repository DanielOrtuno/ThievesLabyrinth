...
Texture2D flashMap : register( t3 );
...
cbuffer UtilityBuffer : register ( b2 )
{
	float4 vFlashColor;
}
...
float4 main(INPUT input) : SV_TARGET
{
	...
	float4 vFlashValue = vFlashColor * vFlashColor.w;


	return ( vBaseColor * saturate(vLightsColor) + vFlashValue ) + vSpecularValue + vBaseColor * vBufferData.y;
}